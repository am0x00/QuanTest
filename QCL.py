...
def QCL_MNIST(N, DEPTH, class_nums):
    """
    Input:
        N: qubits num DEPTH: deepth of Qnn,  class_nums: choose classf_mode
    Output:
        Return the Qnn
    """
    if class_nums == 2:
    ...
    net_state_dict = paddle.load(f"QCL_MNIST{classf_mode}clas_{N}qub_{DEPTH}dep.pdparams")
...
def QCL_Fashion(N, DEPTH, class_nums):
    ...
    net = QCL(n=N, depth=DEPTH)
    net_state_dict = paddle.load(f"QCL_Fashion{classf_mode}clas_{N}qub_{DEPTH}dep.pdparams")
    net.set_state_dict(net_state_dict)
...
if __name__ == '__main__':
    ...
    clfn = int(sys.argv[3])
    qcl_model_softmax(N=qubit_num, DEPTH=qnn_depth, class_nums=clfn)
    # if classf_mode == 2:
