...
def CCQC_MNIST(N, DEPTH, class_nums):
    ...
    net_state_dict = paddle.load(f"CCQC_MNIST{classf_mode}clas_{N}qub_{DEPTH}dep.pdparams")
...
def CCQC_Fashion(N, DEPTH, class_nums):
    ...
    net = CCQC(n=N, depth=DEPTH)
    net_state_dict = paddle.load(f"CCQC_Fashion{classf_mode}clas_{N}qub_{DEPTH}dep.pdparams")
    net.set_state_dict(net_state_dict)
